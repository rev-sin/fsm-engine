    ...
    return (
        <div className={`absolute w-screen h-15 ${isVisible ? 'top-2' : '-top-15'} flex justify-center items-center transition-all ease-in-out duration-500`}>
            <div className="flex justify-center items-center gap-3 w-fit px-2 h-full bg-primary-bg border border-border-bg rounded-xl shadow-[0px_0px_50px_0px_#00000080] select-none">
                ...
                <button onClick={() => setIsVisible(!isVisible)} className={`flex justify-center items-center gap-1 absolute -bottom-10 font-github bg-primary-bg text-white text-sm font-bold px-3 py-2 border border-border-bg rounded-lg cursor-pointer`}>
                    {EngineMode.type}
                    <ChevronDown className={`${isVisible && 'rotate-180'} transition-all ease-in-out duration-500`} size={24} color="#ffffff" />
                </button>
            </div>
        </div>
    )
